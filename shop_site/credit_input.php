...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        // 購入金額と決済時間の取得（仮に固定の値を使用）
        $price = 1000; // 購入金額を適切な値に置き換える
        $paymentTime = date('Y-m-d H:i:s'); // 決済時間を現在の日時に設定

        // データベースにデータを挿入
        $sql = "INSERT INTO credit_card_info (name, card_number, expiration_date, security_code, price, payment_time)
                VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $dbh->prepare($sql);
        $stmt->execute([$name, $cardNumber, $expirationDate, $securityCode, $price, $paymentTime]);
            ...
            <div class="front">
                ...
                <svg version="1.1" id="cardfront" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                    ...
                    <g id="Front">
                        ...
                        <text transform="matrix(1 0 0 1 54.1064 428.1723)" id="svgname" class="st2 st5 st6">TEST NAME</text>
            ...
            <div class="back">
                <svg version="1.1" id="cardback" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                    ...
                    C707.1,246.4,704.4,249.1,701.1,249.1z" />
                        ...
                        <text transform="matrix(1 0 0 1 59.5073 228.6099)" id="svgnameback" class="st12 st13">TEST NAME</text>
                    </g>
                </svg>
            </div>
        </div>
...
document.getElementById("submit-button").addEventListener("click", async function () {
    ...
    // クレジットカード情報のバリデーションなどの処理が必要

     // クレジットカード情報が正常であれば、データベースへの保存とメール処理を行う
// クレジットカード情報が正常であれば、データベースへの保存とメール処理を行う
if (validateCreditCardInfo(name, cardNumber, expirationDate, securityCode)) {
...
try {
    ...
    emailData.append('cardnumber', cardNumber);
    emailData.append('expirationdate', expirationDate);
    emailData.append('securitycode', securityCode);
    // emailData.append('email', email);

                // データベースへの保存処理（PHPへPOSTリクエストを送信）
                const response = await fetch('credit_input.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: `name=${encodeURIComponent(name)}&cardnumber=${encodeURIComponent(cardNumber)}&expirationdate=${encodeURIComponent(expirationDate)}&securitycode=${encodeURIComponent(securityCode)}`,
            });

            if (!response.ok) {
                throw new Error('Failed to save credit card information to the database.');
            }

    // メール送信
    const emailResponse = await fetch('send_credit_email.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
    ...
    const creditInfoEnteredEvent = new Event("creditInfoEntered");
    window.opener.dispatchEvent(creditInfoEnteredEvent);
    // 成功メッセージを表示
...
document.getElementById("cancel-button").addEventListener("click", function () {
    ...
    function validateCreditCardInfo(name, cardNumber, expirationDate, securityCode) {
    // ここにバリデーションのルールを実装する
    ...
    // クレジットカード番号のバリデーションを実装する
    // 例: カード番号が16桁の数字であるかをチェック
    ...
    const expirationYear = parseInt(expirationDateParts[1]) + 2000; // 20xx形式の年に変換
    if (expirationYear < today.getFullYear() || (expirationYear === today.getFullYear() && expirationMonth < (today.getMonth() + 1))) {
        alert("Please enter a valid expiration date.");
        return false;
    }

    // ここに他のバリデーションルールを追加する
...
name.addEventListener('input', function () {
    if (name.value.length == 0) {
        document.getElementById('svgname').innerHTML = 'TEST NAME';
        document.getElementById('svgnameback').innerHTML = 'TEST NAME';
    } else {
